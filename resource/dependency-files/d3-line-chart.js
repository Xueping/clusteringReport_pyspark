(function (root, factory) {
	...
	function LineChart(opts) {
		...
		var lc = {
			...
			title:opts.title ||'Line Title',
			point_size:opts.point_size ||5,
		...
		function plot_title() {
			var title = lc.graph.append("text");
			title.attr("class", "line-title")
			.text(lc.title)
			.attr("x", lc.width/2)
			.attr("y", -5 );
		}
		...
		function plot_legend(series, index) {
			...
			.attr("cy", index*20)
			.attr("r", 5)
			.style("stroke", d3.rgb(lc.color).brighter())
			.style("fill", lc.color)
